...
表情集（5张 PNG/SVG 或 Lottie）：开心大笑、平和微笑、紧张皱眉、伤心下垂、振奋兴奋
...
振奋 (Excited)：H≈300–320（洋红），S 最高，L 中；喷发粒子短尾

在鼠标移动时，对落点格子和邻域 1-ring 做 径向渐变，让“刷子”更有手感。

# 5)关键：情绪 → 钢琴乐理映射（音高/时值/力度/律动）

我们用五类音阶/和声 + 节奏/音长/力度/奏法，并结合你设定的“每几格出一个调”的节拍抽样规则。
...
| -------------- | -------------------------------- | ---- | ----------------------------- |
| 振奋 Excited | **多利亚 Dorian + 大二度跳进** | D4 | 律动感强，允许 9、11 色彩音 |
...
* Excited：2 格一音——快（BPM 135–150），八分切分节奏，力度 85–110（允许峰值），偶发跳进（纯四/五度）。
...
# GET /moods -> 基础配置
{
  ...
  "excited": {"bpm":145, "step":2, "scale":"D_dorian",  "vel":[85,110],"legato":0.8}
}
...
```python
type MoodName = "happy"|"calm"|"tense"|"sad"|"excited";
